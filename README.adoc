...
> Spring's official documentation and examples do not provide runnable example on implementing mutations.
Therefore, I Added a simple mutation example and updated the README.
>
> .Reference links:
...
== Adding code to add data

=== Define the addBook Mutation

First, define a mutation in your GraphQL schema. Open your schema file located at `src/main/resources/graphql/schema.graphqls` and add the following mutation definition:
...
This mutation allows clients to add a new book by providing the name, page count, and associated author ID.

Ensure your controller handles the mutation logic using the `@MutationMapping` annotation:
...
Enter the following mutation in the GraphiQL interface and click the "play" button:

[source,graphql]
mutation AddBookMutation {
  addBook(name: "GraphQL Basics", pageCount: 250, authorId: "42") {
    id
    name
    pageCount
    author {
...
> Note: The author field is returned as null because author is not exists.

Once the book is added, you can query `bookById(id: "book-4")` successfully.
